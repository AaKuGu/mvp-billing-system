...
const SmallScr = ({ isOpen, setIsOpen }) => {
  return (
    <div className={` ${isOpen ? "w-[250px]" : "w-fit"} h-fit `}>
      {!isOpen ? (
        <div className=" flex items-center justify-between p-2  bg-purple-100 text-purple-800">
          <button onClick={() => setIsOpen(true)} className="text-xl">
            <FaBars />
          </button>
        </div>
      ) : (
        <div className=" flex justify-end p-2 flex-col w-full h-screen relative">
